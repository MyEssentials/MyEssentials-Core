...
public class ChatComponentFormattedTest extends MECTest {
    ...
    public void shouldCreateMessageFromStringWithProperStyle() {
        ...
        Assert.assertEquals("Message did not get created properly", "520", message.getUnformattedText());
        Assert.assertEquals("Color was not the correct one for the first argument", TextFormatting.DARK_BLUE, message.getSiblings().get(0).getStyle().getColor());
        Assert.assertEquals("Color was not the correct one for the second argument", TextFormatting.DARK_GREEN, message.getSiblings().get(1).getStyle().getColor());
        Assert.assertTrue("Second argument was not formatted bold", message.getSiblings().get(1).getStyle().getBold());
